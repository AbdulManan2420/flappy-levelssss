...
public class Shark : MonoBehaviour
{
    ...
    Vector3 vel, target, targetCurent, targetRandom, targetFlock;
    ...
    void Hunting()
    {
        if (hunting == false)
        {
            ...
            else if (Physics.CheckSphere(thisTransform.position, huntingZone, layerFlock))
                hunting = true;
        ...
        else
        {
            ...
            if (flockColliders.Length > 0)
                targetFlock = flockColliders[0].transform.position;
    ...
    {
        if (debug)
            Debug.DrawLine(thisTransform.position, targetCurent);
    }
}